...
Label getLabel(const Vec3b& vec) {
	return vec;
}
...
void LabelTable::setLabel(const Vec3b& vec, const Label& lab) {
	table_[vec] = lab;
}

Label LabelTable::getLabel(const Vec3b& vec) {
...
Mat indexSegments(const Mat& image_) {
	Mat result_ = Mat::zeros(image_.rows, image_.cols, CV_8UC3);
			...
			if(nLabs[0] == nLabs[1] && nLabs[0] == nLabs[2] &&
				...
				for(int i = 0; i < 4; ++i)
					if(nLabs[i] != BACKGROUND) {
						if(isLess(nLabs[i], minLab))
							minLab = nLabs[i];
						if(isLess(maxLab, nLabs[i]))
							maxLab = nLabs[i];
					}
				setLabel(result(i, j), minLab);
				table.setLabel(maxLab, table.getLabel(minLab));
			}
		}
	for(int i = 1; i < result.rows-1; ++i)
		for(int j = 1; j < result.cols-1; ++j)
			setLabel(result(i, j), table.getLabel(result(i, j)));
